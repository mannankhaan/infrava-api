...
/** Sent when a NEW operative account is created and assigned a project */
export async function sendWelcomeAndTaskEmail(params: {
  ...
  await sendEmail({
    ...
    html: emailLayout(`
      ...
      ${paragraph('Your account has been created. Use the credentials below to log in and view your assigned project.')}
      ...
      <h3 style="margin:0 0 8px;font-size:16px;color:#172B4D;">Project Assigned</h3>
...
/** Sent when an EXISTING operative is assigned a new project */
export async function sendTaskNotificationEmail(params: {
  ...
  await sendEmail({
    to: params.to,
    subject: `New project assigned — ${params.faultRef}`,
    html: emailLayout(`
      ${heading('New Project Assigned')}
      ${paragraph(`Hi ${params.name},`)}
      ${paragraph('A new project has been assigned to you. Log in to view details and start work.')}
      ${faultDetailsBlock(params)}
      ${primaryButton('View Project', env.APP_URL)}
    `),
  });
}
...
export async function sendFaultRejectedEmail(params: {
  ...
  await sendEmail({
    to: params.to,
    subject: `Project returned — ${params.faultRef} needs revision`,
    html: emailLayout(`
      ${heading('Project Returned for Revision')}
      ${paragraph(`Hi ${params.name},`)}
      ...
      ${primaryButton('Open Project', env.APP_URL)}
    `),
  });
}

/** Sent to admin when operative SUBMITS a project for review */
export async function sendFaultSubmittedEmail(params: {
  ...
  await sendEmail({
    ...
    html: emailLayout(`
      ...
      ${paragraph(`<strong>${params.operativeName}</strong> has submitted their work for <strong>${params.faultRef} — ${params.faultTitle}</strong>. It's now in your Open Projects queue for review.`)}
      ${primaryButton('Review Project', env.APP_URL)}
    `),
  });
}

/** Sent to admin when a project is completed */
export async function sendFaultCompletedEmail(params: {
  ...
  await sendEmail({
    to: params.to,
    subject: `Project completed — ${params.faultRef}`,
    html: emailLayout(`
      ${heading('Project Completed')}
      ${paragraph(`Hi ${params.name},`)}
      ${paragraph(`The project <strong>${params.faultRef} — ${params.faultTitle}</strong> has been marked as completed. The PDF report is available for download in your Reports section.`)}
